$(function () {
      ...
      },
      {
        ...
        "render": function (data, type, row) {
          return row.registryList
              ? '<a class="show_registryList" href="javascript:;" _id="'
              + row.id + '" >'
        ...
        "render": function (data, type, row) {
          return function () {
            ...
            var html = '<div class="btn-group">\n' +
                ...
                + '" >\n' +
                '       <li><a href="javascript:void(0);" class="opt_edit" >'
                + I18n.system_opt_edit + '</a></li>\n' +
                '       <li><a href="javascript:void(0);" class="opt_del" >'
                + I18n.system_opt_del + '</a></li>\n' +
                '     </ul>\n' +
                '   </div>';
    ...
    if (row.registryList) {
      for (var index in row.registryList) {
        html += (parseInt(index) + 1) + '. <span class="badge bg-green" >'
            + row.registryList[index] + '</span><br>';
      }
    }
    html += '</div>';
  ...
  var addModalValidate = $("#addModal .form").validate({
    ...
    submitHandler: function (form) {
      $.post(base_url + "/jobgroup/save", $("#addModal .form").serialize(),
  ...
  // addressType change
  $("#addModal input[name=addressType], #updateModal input[name=addressType]").click(
      function () {
        var addressType = $(this).val();
        var $addressList = $(this).parents("form").find(
  ...
  $("#jobgroup_list").on('click', '.opt_edit', function () {
    ...
    $("#updateModal .form input[name='addressType'][value='" + row.addressType
        + "']").click();
    // 机器地址
  ...
  var updateModalValidate = $("#updateModal .form").validate({
    ...
    submitHandler: function (form) {
      $.post(base_url + "/jobgroup/update", $("#updateModal .form").serialize(),
          function (data, status) {
            if (data.code == "200") {
