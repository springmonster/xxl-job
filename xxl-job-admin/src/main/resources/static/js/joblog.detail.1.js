$(function () {
  ...
  if (!(triggerCode == 200 || handleCode != 0)) {
    ...
    $('#logConsole').append(
        '<span style="color: red;">' + I18n.joblog_rolling_log_triggerfail
        + '</span>');
    return;
  }
  ...
  function pullLog() {
    ...
    if (pullFailCount++ > 20) {
      logRunStop(
          '<span style="color: red;">' + I18n.joblog_rolling_log_failoften
          + '</span>');
      return;
    }
      ...
      success: function (data) {
        ...
        if (data.code == 200) {
          ...
          if (fromLineNum > data.content.toLineNum) {
            ...
            if (data.content.end) {
              logRunStop(
                  '<br><span style="color: green;">[Rolling Log Finish]</span>');
              return;
            }
